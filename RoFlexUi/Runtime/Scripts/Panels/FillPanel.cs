...
namespace RO_Flex_UI.Panels
{
    ...
    public class FillPanel : MonoBehaviour, IPanel
    {
        [Header("References")]
        ...
        public IReadOnlyList<GameObject> Cells => cells;
        ...
        public T GetCell<T>(int index) where T : Component
        {
            var cell = GetCell(index);
            return cell.GetComponent<T>();
        }

        public bool TryGetCell<T>(int index, out T cell) where T : Component
        {
            cell = null;
            ...
            return cells[index].TryGetComponent(out cell);
        }

        public List<T> GetCells<T>() where T : Component
        {
            var result = new List<T>();

            foreach (var cellObject in cells)
            {
                if (cellObject != null && cellObject.TryGetComponent<T>(out var cell))
                    result.Add(cell);
            }
